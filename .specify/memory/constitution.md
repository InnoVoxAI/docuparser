...
2. **Branch Naming**: Feature branches MUST follow the Spec Kit sequential
   convention `###-short-description` (zero-padded 3-digit ordinal, e.g.
   `012-download-map-extraction`), as generated by the `speckit.git.feature`
   tooling; the ordinal MUST match the feature's `docs/specs/###-*` directory.
   Hotfix branches MAY use a `fix/###-short-description` prefix.
