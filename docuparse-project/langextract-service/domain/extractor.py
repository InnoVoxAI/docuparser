...
def extract_fields(
    raw_text: str, layout: str, document_type: str = "unknown"
) -> ExtractedDocument:
...
def _extract_boleto(raw_text: str) -> tuple[dict[str, str | None], float]:
    fields = {
        "linha_digitavel": _first_match(
            raw_text, r"(\d{5}\.?\d{5}\s+\d{5}\.?\d{6}\s+\d{5}\.?\d{6}\s+\d\s+\d{14})"
        ),
        "vencimento": _first_match(raw_text, r"\b(\d{2}[/-]\d{2}[/-]\d{2,4})\b"),
        "valor": _first_match(
            raw_text,
            r"(?:valor|total)\s*[:\-]?\s*(R\$\s*\d{1,3}(?:\.\d{3})*,\d{2}|\d{1,3}(?:\.\d{3})*,\d{2})",
            re.IGNORECASE,
        ),
        "beneficiario": _first_match(
            raw_text,
            r"(?:benefici[aá]rio|cedente)\s*[:\-]?\s*([A-Za-z0-9 .&/-]{3,80})",
            re.IGNORECASE,
        ),
    }
...
def _extract_fatura(raw_text: str) -> tuple[dict[str, str | None], float]:
    fields = {
        ...
        "unidade": _first_match(
            raw_text,
            r"(?:unidade|uc)\s*[:\-]?\s*([A-Za-z0-9 .&/-]{2,40})",
            re.IGNORECASE,
        ),
