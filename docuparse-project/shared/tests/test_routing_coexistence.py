...
def test_local_scheme_reads_from_local_even_in_s3_write_mode(tmp_path):
    with mock_aws():
        ...
        local = LocalStorage(tmp_path)
        s3 = S3Storage(bucket="test-bucket", region="us-east-1", access_key="k", secret_key="s")
...
def test_s3_uri_without_backend_raises_clear_error(tmp_path):
    local = LocalStorage(tmp_path)
    routing = RoutingStorage(write_scheme="local", local=local, s3=None, s3_provider=None)
...
def test_lazy_s3_provider_built_on_demand(tmp_path):
    with mock_aws():
        ...
        routing = RoutingStorage(write_scheme="local", local=local, s3=None, s3_provider=provider)
