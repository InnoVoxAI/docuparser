...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser) -> None:
        parser.add_argument(
            "--stream", required=True, help="DLQ stream, for example ocr.completed.dlq."
        )
        parser.add_argument(
            "--id", required=True, dest="entry_id", help="DLQ entry id."
        )
        parser.add_argument(
            "--target-stream",
            help="Target stream. Defaults to the original stream stored in the DLQ entry.",
        )
        parser.add_argument(
            "--note",
            default="",
            help="Operational note stored in the requeue audit stream.",
        )
        parser.add_argument(
            "--requested-by",
            default="cli",
            help="Operator identifier stored in the audit event.",
        )
        parser.add_argument(
            "--limit",
            type=int,
            default=500,
            help="Maximum DLQ entries to scan when locating the id.",
        )
        parser.add_argument(
            "--execute",
            action="store_true",
            help="Publish the original payload back to the target stream.",
        )
        parser.add_argument(
    ...
    def handle(self, *args, **options) -> None:
        ...
        if not result["execute"]:
            self.stdout.write(
                "Use --execute to publish the original payload back to the target stream."
            )
        elif result["requeued_event_stream_id"]:
            self.stdout.write(
                f"Published as stream id {result['requeued_event_stream_id']}."
            )
