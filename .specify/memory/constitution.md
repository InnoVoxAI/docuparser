...
SYNC IMPACT REPORT
==================
Version change: 1.0.0 → 1.0.1 (PATCH — non-semantic correction of workflow guidance)

Amendment summary:
  - Development Workflow §2 "Branch Naming": corrected the required feature-branch
    format from `feat/###-short-description` to the Spec Kit sequential convention
    `###-short-description` (e.g., 012-download-map-extraction), as generated by the
    `speckit.git.feature` tooling and matching the `docs/specs/###-*` layout and all
    existing features (001–011). Traceability intent is unchanged; only the concrete
    format was corrected. Hotfix branches MAY still use a `fix/###-*` prefix.

Modified principles: none (Core Principles I–IV unchanged)
Added sections: none
Removed sections: none

Templates / references reviewed:
  - ✅ .specify/templates/plan-template.md — uses `[###-feature-name]`; already consistent
  - ✅ .specify/templates/spec-template.md — `Feature Branch: [###-feature-name]`; already consistent
  - ✅ .specify/templates/tasks-template.md — `[###-feature-name]`; already consistent
  - ✅ CLAUDE.md — no branch-naming reference; no change needed
  - Note: this amendment brings the constitution INTO alignment with templates and tooling
    that already used the `###-name` convention (the constitution was the outlier).
...
**Version**: 1.0.1 | **Ratified**: 2026-06-02 | **Last Amended**: 2026-07-13
