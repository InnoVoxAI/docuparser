...
def test_process_document_calls_classifier_with_filename_and_content(
    monkeypatch,
) -> None:
    calls = []
    ...
    monkeypatch.setattr(
        process_module.engine_resolver,
        "get_engine",
        lambda doc_type, selected=None: "fake",
    )
...
def test_process_document_routes_digital_pdf_to_docling(monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        ...
        {
            "docling": FakeEngine(
                {"raw_text": "texto pdf", "_meta": {"engine": "docling"}}
            ),
            "openrouter": FakeEngine(
                {"raw_text": "wrong", "_meta": {"engine": "openrouter"}}
            ),
        },
    )
    monkeypatch.setattr(process_module, "FieldExtractor", FakeExtractor)
...
def test_process_document_routes_image_pdf_to_openrouter(monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        ...
        {
            "docling": FakeEngine(
                {"raw_text": "wrong", "_meta": {"engine": "docling"}}
            ),
            "openrouter": FakeEngine(
                {"raw_text": "texto imagem", "_meta": {"engine": "openrouter"}}
            ),
        },
    )
    monkeypatch.setattr(process_module, "FieldExtractor", FakeExtractor)
    ...
    assert (
        result["classification_engine_preprocessing_hints"]["openrouter"]
        == "render_pdf_or_image_for_vision_ocr"
    )
    assert result["raw_text"] == "texto imagem"


def test_process_document_fallback_does_not_use_undefined_ocr_result(
    monkeypatch,
) -> None:
    monkeypatch.setattr(
        process_module, "classify_document", lambda filename, content: "scanned_image"
    ...
    monkeypatch.setattr(
        process_module.engine_resolver,
        "get_engine",
        lambda doc_type, selected=None: "primary",
    )
    monkeypatch.setattr(
        process_module,
        "ENGINE_REGISTRY",
        {
            ...
            "tesseract": FakeEngine(
                {
                    "raw_text": "fallback text with enough tokens",
                    "_meta": {"text_length": 32},
                }
            ),
        },
    )
...
def test_openrouter_image_uses_key_values_when_extracted_text_is_empty(
    monkeypatch,
) -> None:
    ...
    fixture = (
        ...
        / "docs_teste"
        / "PHOTO-2026-01-08-18-44-00.jpg"
    )
    result = openrouter_engine.OpenRouterOCREngine().process(
...
def test_openrouter_image_retries_with_qwen_when_text_is_empty(monkeypatch) -> None:
    ...
    def fake_call_openrouter(
        image_bgr, page_label="image_1", timeout_s=120, model_override=None
    ):
