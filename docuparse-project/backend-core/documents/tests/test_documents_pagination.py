...
class DocumentsPaginationTests(TestCase):
    ...
    def test_status_single_filter(self) -> None:
        ...
        data = self.client.get(
            reverse("documents-inbox"), {"status": "APPROVED"}
        ).json()
        assert data["count"] == 1
    ...
    def test_status_csv_filter_buckets(self) -> None:
        ...
        _make_document(
            self.tenant, status=Document.Status.OCR_COMPLETED, filename="b.pdf"
        )
    ...
    def test_search_by_filename(self) -> None:
        ...
        data = self.client.get(
            reverse("documents-inbox"), {"search": "nota-fiscal"}
        ).json()
        assert data["count"] == 1
    ...
    def test_search_by_document_type_and_channel(self) -> None:
        _make_document(
            self.tenant, filename="x.pdf", document_type="boleto", channel="email"
        )
        _make_document(
            self.tenant, filename="y.pdf", document_type="nota", channel="manual"
        )
        by_type = self.client.get(
            reverse("documents-inbox"), {"search": "boleto"}
        ).json()
        assert by_type["count"] == 1
        by_channel = self.client.get(
            reverse("documents-inbox"), {"search": "email"}
        ).json()
    ...
    def test_search_inside_extraction_fields(self) -> None:
        ...
        _make_document(
            self.tenant, filename="other.pdf", fields={"x": {"value": "zzz"}}
        )
    ...
    def test_search_maps_status_label(self) -> None:
        ...
        _make_document(self.tenant, status=Document.Status.RECEIVED, filename="b.pdf")
        data = self.client.get(
            reverse("documents-inbox"), {"search": "aprovado"}
        ).json()
        assert data["count"] >= 1
    ...
    def test_search_resets_to_full_set(self) -> None:
        ...
        data = self.client.get(
            reverse("documents-inbox"), {"search": "unique-marker"}
        ).json()
        assert data["count"] == 1
...
class DocumentFileAuthTests(TestCase):
    ...
    def test_user_without_permission_is_forbidden(self) -> None:
        other = get_user_model().objects.create_user(
            username="nofileperm", password="x"
        )
        self.client.force_authenticate(user=other)
