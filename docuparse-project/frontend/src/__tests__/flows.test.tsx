...
function mockSession(documents: unknown[] = []) {
    server.use(
        ...
        http.get('/api/ocr/documents', ({ request }) =>
            HttpResponse.json(paginatedDocuments(documents as Record<string, unknown>[], request.url)),
        ),
...
describe('Dashboard — documento rejeitado', () => {
    it('abre o modal de rejeição e dispara reprocessamento', async () => {
        ...
        mockSession([
            doc({
                id: 'dR',
                original_filename: 'rejeitado.pdf',
                status: 'REJECTED',
                rejection_notes: 'Documento ilegível',
            }),
        ])
...
describe('Configurações — áreas e salvamento', () => {
    it('navega entre OCR/Email/Integrações e dispara os respectivos saves', async () => {
        ...
        server.use(
            ...
            http.patch('/api/ocr/settings/ocr', () => {
                patched.push('ocr')
                return HttpResponse.json({})
            }),
            http.patch('/api/ocr/settings/email', () => {
                patched.push('email')
                return HttpResponse.json({})
            }),
            http.patch('/api/ocr/settings/integrations', () => {
                patched.push('integrations')
                return HttpResponse.json({})
            }),
        )
        mockSession([])
                ...
                HttpResponse.json([
                    {
                        id: 'u9',
                        name: 'Maria',
                        email: 'maria@docuparse.local',
                        role: { id: 'r1', name: 'Operador' },
                        is_active: true,
                    },
                ]),
            ),
...
describe('Upload', () => {
    it('envia um documento manual e mostra a confirmação', async () => {
        ...
        server.use(http.post('/com/api/v1/documents/manual', () => HttpResponse.json({ document_id: 'doc-123' })))
        mockSession([])
...
describe('Visualização do documento (ação de olho)', () => {
    ...
    it('exibe estado de erro quando o arquivo não carrega', async () => {
        ;(URL as unknown as { createObjectURL: () => string }).createObjectURL = vi.fn(() => 'blob:preview')
        ;(URL as unknown as { revokeObjectURL: () => void }).revokeObjectURL = vi.fn()
        server.use(http.get('/api/ocr/documents/:id/file', () => new HttpResponse(null, { status: 404 })))
        mockSession([
            doc({
                id: 'd1',
                original_filename: 'nota-fiscal.pdf',
        ...
        expect(await screen.findByText('Informações do documento')).toBeInTheDocument()
        await waitFor(() =>
            expect(screen.getByText(/Nao foi possivel carregar a pre-visualizacao/i)).toBeInTheDocument(),
        )
    })
})
